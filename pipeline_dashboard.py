"""Woo Domain Expert — Pipeline Dashboard.
...
_RESEARCH_SECTION_TITLES = (
    ...
    "Woo app behaviour findings from local RAG:",
...
def _get_repo_branches() -> list[str]:
    import config
    repo_path = getattr(config, "WOO_AUTOMATION_REPO_PATH", "")
...
def _extract_release_label(list_name: str) -> str:
    ...
    if not list_name:
        return ""
    match = re.search(r"(woo\w*\s+[\d.]+)", list_name, re.IGNORECASE)
    if match:
...
def _find_existing_tc_comment(card: Any) -> str:
    ...
    for comment in getattr(card, "comments", []) or []:
        if "📋 **QA Test Cases" in comment or "## Woo QA Test Cases" in comment:
            return comment
    return ""
...
def main() -> None:
    ...
    if not st.session_state.get("code_paths_initialized"):
        if config.WOO_AUTOMATION_REPO_PATH:
            st.session_state["automation_code_path"] = config.WOO_AUTOMATION_REPO_PATH
        if config.WOO_PLUGIN_REPO_PATH:
            st.session_state["woo_server_path"] = config.WOO_PLUGIN_REPO_PATH
        if config.WOO_SHIPPING_PRO_REPO_PATH:
            st.session_state["woo_client_path"] = config.WOO_SHIPPING_PRO_REPO_PATH
    ...
    with st.sidebar:
        ...
        _woo_server_cnt = _idx_stats.get("woo_plugin", 0)
        _woo_client_cnt = _idx_stats.get("woo_shipping_pro", 0)
        _woo_cnt = _woo_server_cnt + _woo_client_cnt
        _auto_sync  = _idx_stats.get("automation_sync", {})
        _woo_server_sync = _idx_stats.get("server_sync", {})
        _woo_client_sync = _idx_stats.get("client_sync", {})
        ...
        _woo_server_tag = (
            f" · server `{_woo_server_sync.get('commit', '')}`"
            if _woo_server_sync.get("commit")
            else ""
        )
        _woo_client_tag = (
            f" · client `{_woo_client_sync.get('commit', '')}`"
            if _woo_client_sync.get("commit")
            else ""
        )
        ...
        st.markdown(
            ...
            f"<b>🏪 Woo App:</b> {_sync_badge(_woo_cnt, {})}{_woo_server_tag}{_woo_client_tag}<br>"
        ...
        with st.expander("✍️ Automation Code"):
            auto_path = st.text_input(
                ...
                placeholder="/path/to/ups-woo-automation",
            )
            auto_info = {}
            if auto_path:
                try:
        ...
        # ── Woo App Code (single repo — server + client) ─────────────────
        with st.expander("🏪 Woo App Code"):
            woo_server_path = st.text_input(
                "Backend path",
                key="woo_server_path",
                placeholder="/path/to/plugins/woocommerce-plugins/server/src/shared",
            )
            woo_client_path = st.text_input(
                "Frontend path",
                key="woo_client_path",
                placeholder="/path/to/plugins/woocommerce-plugins/client/src",
            )
            woo_info = {}
            woo_repo_probe = woo_server_path or woo_client_path
            if woo_repo_probe:
                try:
                    from rag.code_indexer import get_repo_info
                    woo_info = get_repo_info(woo_repo_probe)
                except Exception:  # noqa: BLE001
                    pass
            woo_branches = woo_info.get("branches", [])
            woo_current  = woo_info.get("current_branch", "")
            woo_commit   = woo_info.get("commit", "")
            if woo_branches:
                _woo_idx = woo_branches.index(woo_current) if woo_current in woo_branches else 0
                st.selectbox("Branch to pull", woo_branches, index=_woo_idx, key="woo_branch_select")
                st.caption(f"Current: `{woo_current}` @ `{woo_commit}`")
            elif woo_repo_probe:
                st.caption(f"Current: `{woo_current or 'unknown'}` @ `{woo_commit}`" if woo_current else "⚠️ Branch info unavailable")
            st.caption(
                f"Indexed chunks: the Multi-Carrier plugin repo `{_woo_server_cnt}` · the Shipping Pro repo `{_woo_client_cnt}`"
            )
            col_sync, col_idx = st.columns(2)
            with col_sync:
                if st.button("Pull & Sync", key="woo_sync_btn"):
                    from rag.code_indexer import sync_from_git
                    branch = st.session_state.get("woo_branch_select", woo_current or "main")
                    with st.spinner("Syncing…"):
                        sync_results = []
                        if woo_server_path:
                            sync_results.append(
                    ...
                    if not sync_results:
                        st.warning("Set the Multi-Carrier plugin repo and/or client path first.")
                        ...
                        st.success("Synced plugin source app code.")
                    st.rerun()
            with col_idx:
                if st.button("Full Re-index", key="woo_reindex_btn"):
                    from rag.code_indexer import index_codebase
                    with st.spinner("Re-indexing…"):
                        reindex_results = []
                        if woo_server_path:
                    ...
                    if not reindex_results:
                        ...
                        st.success("Re-indexed plugin source app code.")
                    st.rerun()

        # ── Woo Wiki (documents) ─────────────────────────────────────────
        with st.expander("📖 Woo Wiki"):
            wiki_path = st.text_input(
                "Wiki path", key="wiki_path",
                placeholder="/path/to/woo-wiki/wiki",
            )
    ...
    st.markdown(
        '<div class="pipeline-header">'
        '<h1>🚚 Woo QA Pipeline</h1>'
        '<p>AI-powered QA orchestration for Woo carriers</p>'
        '</div>',
        unsafe_allow_html=True,
    )
                            ...
                            for card in cards:
                                ...
                                try:
                                    if not dry_run:
                                        ...
                                        tc.add_comment(
                                            card.id,
                                            f"Moved to {move_target} by Woo QA Pipeline",
                                        )
                                    moved += 1
                                except Exception as exc:
                ...
                with st.expander("🎯 Quick Custom Test", expanded=True):
                    ...
                    if _qct_is_running:
                        ...
                        _qct_url = st.text_input(
                            ...
                            key="qct_url_edit",
                            placeholder="https://your-site.example.com/wp-admin/admin.php?page=ph_multi_carrier_admin_menu",
                        )
                ...
                with st.expander(f"{_status_icon} {card.name}{_approved_badge}{_done_badge}", expanded=False):
                    ...
                    # ── Step 1a.1: Toggle prerequisites ───────────────────
                        ...
                        if _detected_toggles:
                            ...
                            _store_url = woo_admin.dashboard_url(_store_name) if _store_name else ""
                    ...
                    if show_ai_stage:
                        ...
                        st.caption(
                            "Claude opens Chrome, uses automation navigation + Woo domain knowledge, "
                            "walks the live WooCommerce app, verifies each generated test case, and asks QA only when needed. "
                            "Non-WooCommerce platform cards remain manual for AI QA until those browser flows are added."
                        )
                        ...
                        try:
                            from pipeline.card_processor import woo_automation_scope as _aiqa_platform_scope
                            ...
                            _aiqa_supported, _aiqa_platforms, _aiqa_platform_reason = True, ["WooCommerce"], ""

                        if not _aiqa_supported:
                            st.info(_aiqa_platform_reason)
                            st.checkbox(
                                "QA confirmed: test this shared Woo behavior using the available WooCommerce AI QA flow",
                                key=_platform_confirm_key,
                                help="Use this only after QA confirms the non-WooCommerce reported card should still be executed through the current WooCommerce browser flow.",
                            )
                        ...
                        _expert_default = (
                            f"For card '{card.name}', what are the expected Woo behavior, prerequisites, "
                        ...
                        if _ask_clicked and _expert_question.strip():
                            with st.spinner("Querying Woo domain knowledge…"):
                                try:
                    ...
                    if show_automation_stage:
                        with _automation_container:
                            ...
                            st.caption(
                                ...
                                "Automation generation currently targets the WooCommerce Woo Playwright repo only."
                            )
                            ...
                            try:
                                from pipeline.card_processor import woo_automation_scope as _woo_auto_scope

                                _auto_supported, _auto_platforms, _auto_platform_reason = _woo_auto_scope(
                                    card.name,
                                    card.desc or "",
                                    _auto_tc_src,
                                    "\n".join(getattr(card, "comments", []) or []),
                                ...
                                _auto_supported, _auto_platforms, _auto_platform_reason = True, ["WooCommerce"], ""
                            ...
                            if not _is_approved_for_auto:
                                ...
                                st.checkbox(
                                    "QA confirmed: generate WooCommerce automation for this shared behavior",
                                    key=_auto_platform_confirm_key,
                                    help="Use this only after QA confirms the reported non-WooCommerce card should be covered through the current WooCommerce automation repo.",
                                )
                            else:
                                ...
                                _repo_path = getattr(config, "WOO_AUTOMATION_REPO_PATH", "")
                                ...
                                if _has_ai_qa_context:
                                    ...
                                    _use_live_explore = st.checkbox(
                                        ...
                                        help=(
                                            "Navigates the real Woo app and captures UI elements. "
                                ...
                                if _use_live_explore:
                                    ...
                                    with st.expander("🌐 Explore live app", expanded=False):
                                        ...
                                        if st.button("🌐 Explore App", key=f"auto_explore_btn_{card.id}", use_container_width=True):
                                            with st.spinner("Exploring live Woo app…"):
                                                try:
                ...
                if show_automation_stage and card is cards[-1]:
                    ...
                    with _bug_col1:
                        _bug_feature = st.text_input(
                            ...
                            placeholder="e.g. Orders -> order edit screen, Settings -> Shipping, Products -> Shipping tab",
    ...
    with tab_new_carrier:
        ...
        with _load_col2:
            if st.button(
                ...
                try:
                    ...
                    st.session_state["new_carrier_woo_site_url"] = _loaded_run.woo_site_url
                    ...
                    st.session_state["new_carrier_woo_consumer_secret"] = _loaded_run.woo_consumer_secret
                    st.session_state["new_carrier_woo_api_version"] = _loaded_run.woo_api_version
        ...
        def _persist_new_carrier_run(*, notes: str = "") -> str:
            ...
            _run = build_new_carrier_run(
                ...
                woo_site_url=st.session_state.get("new_carrier_woo_site_url", "").strip(),
        ...
        _env_dir = Path(config.WOO_AUTOMATION_REPO_PATH) / "carrier-envs"
        ...
        if _current_store_slug:
            _derived_woo_site_url = woo_admin.dashboard_url(_current_store_slug)
            _derived_app_url = f"{_derived_woo_site_url}/apps/woo-qa"
            if not st.session_state.get("new_carrier_woo_site_url", "").strip() or "<store>" in st.session_state.get("new_carrier_woo_site_url", ""):
                st.session_state["new_carrier_woo_site_url"] = _derived_woo_site_url
        ...
        with _setup_col2:
            ...
            _woo_site_url = st.text_input(
                "WordPress admin URL",
                key="new_carrier_woo_site_url",
                placeholder="https://your-site.example.com/wp-admin/",
            ).strip()
                ...
                if not _store_name:
                    ...
                    with st.spinner("Creating WooCommerce store and installing the Woo app..."):
                        try:
                            ...
                            st.session_state["new_carrier_woo_site_url"] = _onboarding.woo_site_url
        ...
        _template_values = {}
        _template_env = ""
        _woo_consumer_secret = ""
        _woo_api_version = ""
        _store_specific_token_ready = bool(st.session_state.get("new_carrier_woo_consumer_secret", "").strip())
                ...
                if not _carrier_code or not _store_name:
                    ...
                    try:
                        ...
                        _run = NewCarrierValidationRun(
                            ...
                            woo_site_url=_woo_site_url or _template_values.get("WOOURL", ""),
                            partner_url=st.session_state.get("new_carrier_partner_stores_url", "").strip()
                            or _template_values.get("PARTNER_URL", ""),
                            partner_apps_url=st.session_state.get("new_carrier_partner_apps_url", "").strip(),
                            user_email=_template_values.get("USER_EMAIL", ""),
        ...
        st.caption("Run the generated carrier env against the existing Playwright suite tags in `ups-woo-automation`.")
        ...
        with _runner_col2:
            _runner_headed = st.checkbox(
                ...
                help="Matches the existing WooCommerce login flow used in automation.",
            )
    ...
    with tab_handoff:
        ...
        if not handoff_cards:
            ...
            _pkg_release = handoff_release or st.session_state.get("hd_list_name", "") or "Woo Release"
                ...
                with _act1:
                    ...
                    if st.button("📎 Attach PDF to Trello", key=f"attach_trello_{doc_type}_{state_suffix}", use_container_width=True, disabled=_attach_disabled):
                        try:
                            ...
                            _trello.add_comment(
                                _target_id,
                                f"📘 {label} attached by Woo QA Pipeline\n\n"
