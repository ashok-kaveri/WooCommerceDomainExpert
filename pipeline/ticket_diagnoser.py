"""Ticket diagnosis helper for Woo customer issues and feature cards."""
...
DIAGNOSIS_PROMPT = dedent("""\
    You are a senior Woo support engineer, QA lead, and domain expert.

    Diagnose this customer issue or feature card using Woo domain knowledge, code context,
    ...
    {{
      ...
      "likely_root_cause": "generic_platform" | "carrier_setup" | "packaging_or_product_data" | "woo_sync" | "request_or_label_api" | "code_defect" | "carrier_side" | "unclear",
...
DIAGNOSIS_MINIMAL_RETRY_PROMPT = dedent("""\
    You are an Woo QA diagnosis assistant.
...
def _basic_fallback_diagnosis(ticket_text: str, *, error: str = "") -> TicketDiagnosis:
    ...
    if not next_checks:
        next_checks.append("Review card requirements, linked references, and the primary affected Woo flow.")
    if not strategy:
        strategy.append("Validate the primary flow first, then run regression checks on nearby app behavior.")

    summary = (
        "Fallback diagnosis: review the primary Woo flow and confirm prerequisites before deep investigation."
    )
...
def diagnose_ticket(ticket_text: str, model: str | None = None) -> TicketDiagnosis:
    ...
    try:
        ...
        be_docs = search_code(ticket_text, k=3, source_type="woo_plugin") or []
        fe_docs = search_code(ticket_text, k=3, source_type="woo_shipping_pro") or []
