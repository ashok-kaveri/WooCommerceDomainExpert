"""Shared Woo carrier knowledge and card scope detection."""
...
CARRIER_PROFILES: tuple[CarrierProfile, ...] = (
    CarrierProfile(
        ...
        support_summary="FedEx in Woo commonly touches special services, customs/commercial invoice data, and request/label payload accuracy.",
    ...
    CarrierProfile(
        ...
        automation_ready=True,
        support_summary="Australia Post flows usually follow the shared Woo order and label path with carrier-specific account/setup checks.",
    ),
    CarrierProfile(
    ...
    CarrierProfile(
        ...
        automation_ready=True,
        support_summary="PostNord scenarios are usually carrier-specific label/customs/document behavior on top of shared Woo flows.",
    ),
    CarrierProfile(
        ...
        automation_ready=True,
        support_summary="TNT Express is generally a carrier-specific international/service workflow layered onto shared Woo order processing.",
    ),
    CarrierProfile(
    ...
    CarrierProfile(
        ...
        automation_ready=True,
        support_summary="Post NL uses shared Woo flows with carrier-specific service and label expectations.",
    ),
    CarrierProfile(
        ...
        automation_ready=True,
        support_summary="Aramex scenarios usually require carrier-specific setup and request/label validation within shared Woo navigation.",
    ),
    CarrierProfile(
        ...
        automation_ready=True,
        support_summary="NZ Post follows the standard Woo navigation with carrier-specific rating/label/account rules.",
    ),
    CarrierProfile(
...
def carrier_prompt_block(*texts: str) -> str:
    ...
    if scope.is_generic:
        return (
            ...
            f"No explicit carrier was detected, so the card should be treated as a generic Woo flow. "
            ...
            "Platform note: Woo supports many carriers globally; local automation-ready carriers currently include "
            f"{automation_ready}."
        )
    lines = [
    ...
    if primary:
        ...
        if primary.typical_areas:
            lines.append("Typical Woo areas: " + ", ".join(primary.typical_areas))
...
def carrier_research_context(*texts: str, max_docs: int = 4) -> str:
    ...
    if scope.is_generic:
        sections.append(
            "Generic execution rule: prefer shared Woo flows first "
...
def get_default_store_slug(*texts: str) -> str:
    """Return the preferred WooCommerce store slug for a card/context.
    ...
    if target_profile and target_profile.internal_code:
        try:
            ...
            store = _read_env_value(env_path, "WOO_SITE_URL")
            if store:
                return store
        except Exception:
            pass

    # Fall back to the automation repo .env (WOO_SITE_URL there is authoritative)
    _auto_env = Path(getattr(config, "WOO_AUTOMATION_REPO_PATH", "") or "").parent / ".env"
    if not _auto_env.exists():
        _auto_env = Path(getattr(config, "WOO_AUTOMATION_REPO_PATH", "") or "") / ".env"
    store = _read_env_value(_auto_env, "WOO_SITE_URL") if _auto_env.exists() else ""
    if store:
        return store.removesuffix(".wp-admin")

    raw = (getattr(config, "STORE", "") or "").strip()
    return raw.removesuffix(".wp-admin")
...
def get_default_app_url(*texts: str) -> str:
    ...
    if not store:
        return ""
    return woo_admin.plugin_settings_url()
