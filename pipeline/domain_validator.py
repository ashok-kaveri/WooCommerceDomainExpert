"""
pipeline/domain_validator.py — Domain validator for Woo QA Pipeline.
...
VALIDATION_PROMPT = dedent("""\
    You are a senior domain expert and QA lead for the Woo (Multi-Carrier Shipping Labels)
    WooCommerce App built by PluginHive. The app supports multiple carriers: FedEx, UPS, DHL, USPS.

    The WooCommerce label flow: Orders list → open the order → Generate Packages →
    ...
    Interpretation rule:
    - If no carrier is explicitly named in the card, validate it as a generic Woo platform behaviour first.
    ...
    {{
      ...
      "accuracy_issues": [...],
      "suggestions": [...],
      "kb_insights": "<key facts, constraints, known Woo behaviours for this feature>"
    }}
""")

VALIDATION_FIX_PROMPT = dedent("""\
    You are a senior domain expert for PluginHive's WooCommerce shipping plugins on WordPress/WooCommerce.
    ...
    - Keep all scenarios testable through the Woo app/UI/API flow.
    - Do not invent unsupported carrier rules.
    - Preserve the platform named by the card or AC. If no platform is explicit, default QA/support wording to WooCommerce.
...
VALIDATION_MINIMAL_RETRY_PROMPT = dedent("""\
    You are a QA domain validator for PluginHive's WooCommerce shipping plugins on WordPress/WooCommerce.
...
def validate_card(
    ...
    """Validate a Trello card's requirements against the Woo knowledge base.
