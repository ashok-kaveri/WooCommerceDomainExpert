"""
AI QA Agent — PluginHive WooCommerce Edition
============================================
Adapted from MCSLDomainExpert/pipeline/smart_ac_verifier.py. The pipeline shape
is shared; the browser model is not.
...
Five-stage pipeline per scenario:
  ...
  4. Agentic browser loop — up to MAX_STEPS=15
  5. Pass / fail / partial / qa_needed verdict with screenshot evidence

WooCommerce specifics — the things that differ from MCSL:
  - Multi-carrier support: carrier detected from AC text, injected into the plan
  - CARRIER_CODES map — FedEx C2, UPS C3, DHL C1, USPS C22, etc.
  - **No app iframe and no single app endpoint.** The plugins render inline in
    wp-admin, and every screen has its own URL, so navigation is a goto.
  - Label flow: Orders list → order edit screen → Generate Packages →
    Calculate Rates → pick service → Confirm Shipment → Print Label
  - Rate evidence must be captured between Calculate Rates and Confirm Shipment;
    confirming replaces the rate list with the shipment result.
"""
...
_AUTH_JSON = (
    Path(getattr(config, "WOO_CHROME_AUTH_PATH", ""))
    if getattr(config, "WOO_CHROME_AUTH_PATH", "")
    else (
        Path(config.WOO_AUTOMATION_REPO_PATH) / "auth-chrome.json"
        if getattr(config, "WOO_AUTOMATION_REPO_PATH", "")
...
def _store_slug(raw: str = "") -> str:
    """Return the bare WooCommerce store slug (no .wp-admin suffix).

    STORE env var may be set to 'mystore.wp-admin' or just 'mystore'.
    wp-admin URLs are built from the site origin: <site>/wp-admin/admin.php?page=...
    """
    s = (raw or getattr(config, "STORE", "")).strip()
    return s.removesuffix(".wp-admin")
...
def _platform_confirmation_report(
    ...
    *,
    ...
    supported, platforms, reason = woo_automation_scope(
        card_name,
        text,
        app_url,
    ...
    return VerificationReport(
        ...
        scenarios=[
            ScenarioResult(
                ...
                finding=reason,
                qa_question=(
                    f"{reason} Reply with confirmation if this shared Woo feature should be tested through "
                    "the available WooCommerce browser flow."
                ),
            )
        ],
    )
...
_WOO_VERIFY_KEYWORDS = frozenset({
    "tracking number", "tracking no", "fulfilled in woo", "woo fulfillment",
    "fulfill order", "woo order", "woo orders",
})

_WOO_PRODUCT_KEYWORDS = frozenset({
...
def _normalise_nav_destination(destination: str) -> str:
    """Map model/QA wording to a supported Woo navigation key."""
...
def _get_carrier_config_steps(carrier_name: str, action: str = "add") -> list[str]:
    ...
    """
    base_steps = [
        "Go to wp-admin > Multi Carrier (admin.php?page=ph_multi_carrier_admin_menu)",
        f"Open the {carrier_name} registration screen "
        f"(admin.php?page=ph_multi_carrier_{carrier_name.lower()}_registration)",
    ]
    if action == "add":
        base_steps += [
            f"Enter the {carrier_name} account credentials "
            "(account number, and the carrier's API key/secret or client id/secret)",
            "Click Save / Register",
            f"Verify the screen reports {carrier_name} as registered",
            "Confirm the method is enabled on the shipping zone under test "
...
def _get_preconditions(
    ...
    Each branch returns an ordered list of step strings that reference the
    WooCommerce label flow (Orders → order edit screen → Generate Packages →
    Calculate Rates → Confirm Shipment). CLEANUP steps are appended as explicit
    list items (not comment strings) so the agent executes them afterwards.

    Args:
        scenario_text: Scenario text (used for keyword matching).
        carrier:       Carrier name (e.g. 'fedex', 'FedEx', 'ups').
        app_base:      Site URL of the QA WooCommerce store (optional).
    ...
    # Dry ice, alcohol, battery, signature are per-product fields the plugin adds
    # to the product's Shipping tab. There is no separate in-app product screen.
    product_nav = [
        "navigate: products  (wp-admin > Products)",
        "Open the test product",
        "Open the 'Shipping' tab in the Product data box",
        "Scroll to the PluginHive shipping fields added by the plugin",
    ]

    # --- WOOCOMMERCE LABEL FLOW ---
    # Grounded in ups-woo-automation/src/pages/wooCommerceAdmin/ordersPage.ts.
    # index 0-1: reach the order edit screen
    # index 2-3: build packages and rate them
    # index 4-5: pick the service and confirm the shipment
    # index 6:   evidence
    label_flow = [
        "navigate: orders  (admin.php?page=wc-orders)",
        "Open the test order (a.order-view), or go straight to "
        "admin.php?page=wc-orders&action=edit&id=<order_id>",
        "In the plugin's shipment metabox, click 'Generate Packages' "
        "(.button.ups_generate_packages) → the package table renders",
        "Click 'Calculate Rates' (.button.wf_ups_generate_packages_rates) → wait for "
        "'Step 2: Initiate your shipment.'",
        "Select the service in #wf_ups_service_select",
        "Click 'Confirm Shipment' (.button.ups_create_shipment)",
        "Verify the 'Print Label' link appears and the order note records the tracking number",
    ]

    save_steps = [
        "Click 'Update' (product) or 'Save changes' (settings)",
        "Verify the WordPress success notice (.notice.notice-success) appears",
    ]
    ...
    if carrier_lower == "fedex":
        if "dry ice" in lower:
            ...
            cleanup = [
                "(CLEANUP — after the shipment is confirmed) Go back to the product",
    ...
    # --- DHL ---
        ...
        if "insurance" in lower:
            ...
            steps = label_flow + [
                "Verify the commercial invoice is generated once the shipment is confirmed",
...
def _infer_setup_requirements(scenario: str, carrier_name: str, app_base: str = "") -> dict[str, Any]:
    ...
    is_woo_verify = any(kw in lower for kw in _WOO_VERIFY_KEYWORDS)
    is_woo_product = any(kw in lower for kw in _WOO_PRODUCT_KEYWORDS)
    ...
    if is_product_setup:
        ...
        precondition_steps.extend([
            "Open the product in wp-admin > Products.",
    ...
    if is_document_check:
        ...
        _add_look("Print Label link is available on the order screen", "expected document type is generated")
        if order_action == "none":
            order_action = "existing_fulfilled" if "download" in lower else "create_new"

    if is_woo_verify:
        _add_nav("wooorders")
        _add_look("WooCommerce order shows fulfilled state", "tracking number appears on the WooCommerce order")

    if is_woo_product:
        _add_nav("wooproducts")
        precondition_steps.extend([
            "Open WooCommerce Products admin.",
            "Create or edit the WooCommerce product needed for this scenario.",
            "Capture the product id and variant id after save.",
        ])
        _add_look("product appears in WooCommerce products admin", "product id / variant id can be retrieved for ordering")

    if "generate label" in lower or "label" in lower or "shipment" in lower:
        _add_nav("orders")
        _add_api("/rates", "/label", "/shipment")
        _add_look("shipment is confirmed and Print Label appears", "label/rate request includes expected scenario data")
        if order_action == "none":
            order_action = "create_new"
    ...
    if any(kw in lower for kw in ("multiple package", "multi package", "multi-package",
        ...
        _add_nav("orders")
        _add_look(
            "the failed package row on the order screen offers a cleanup / reset action",
...
# WooCommerce navigation map.
#
# Unlike the Shopify MCSL app (one endpoint, click-only navigation inside an
# iframe), WooCommerce admin is fully URL-addressable. Every destination below
# is a real wp-admin or storefront URL, so navigation is a goto, not a click
# chain. Paths are the ones ups-woo-automation's page objects use.
_Woo_NAV_MAP: dict[str, dict] = {
    # ── WooCommerce admin ────────────────────────────────────────────
    "orders":          {"type": "admin", "path": "admin.php", "params": {"page": "wc-orders"}},
    "wooorders":       {"type": "admin", "path": "admin.php", "params": {"page": "wc-orders"}},
    "products":        {"type": "admin", "path": "edit.php", "params": {"post_type": "product"}},
    "wooproducts":     {"type": "admin", "path": "edit.php", "params": {"post_type": "product"}},
    "appproducts":     {"type": "admin", "path": "edit.php", "params": {"post_type": "product"}},
    "newproduct":      {"type": "admin", "path": "post-new.php", "params": {"post_type": "product"}},
    "status":          {"type": "admin", "path": "admin.php", "params": {"page": "wc-status"}},
    "plugins":         {"type": "admin", "path": "plugins.php", "params": {}},
    "dashboard":       {"type": "admin", "path": "", "params": {}},

    # ── WooCommerce settings ─────────────────────────────────────────
    "settings":        {"type": "admin", "path": "admin.php", "params": {"page": "wc-settings"}},
    "generalsettings": {"type": "admin", "path": "admin.php", "params": {"page": "wc-settings", "tab": "general"}},
    "shipping":        {"type": "admin", "path": "admin.php", "params": {"page": "wc-settings", "tab": "shipping"}},
    "shippingzones":   {"type": "admin", "path": "admin.php", "params": {"page": "wc-settings", "tab": "shipping"}},
    "shippingrates":   {"type": "admin", "path": "admin.php", "params": {"page": "wc-settings", "tab": "shipping"}},
    "shippingclasses": {"type": "admin", "path": "admin.php",
                        "params": {"page": "wc-settings", "tab": "shipping", "section": "classes"}},

    # ── PluginHive plugin screens ────────────────────────────────────
    "pluginsettings":  {"type": "admin", "path": "admin.php", "params": {"page": woo_admin.MULTI_CARRIER_MENU}},
    "carriers":        {"type": "admin", "path": "admin.php", "params": {"page": woo_admin.MULTI_CARRIER_MENU}},
    "packaging":       {"type": "admin", "path": "admin.php", "params": {"page": woo_admin.MULTI_CARRIER_MENU}},
    "labelsettings":   {"type": "admin", "path": "admin.php", "params": {"page": woo_admin.MULTI_CARRIER_MENU}},
    "fedexregistration": {"type": "carrier_registration", "carrier": "fedex"},
    "upsregistration":   {"type": "carrier_registration", "carrier": "ups"},
    "uspsregistration":  {"type": "carrier_registration", "carrier": "usps"},

    # ── Storefront (buyer-side rate checks) ──────────────────────────
    "shop":            {"type": "front", "path": "shop/"},
    "cart":            {"type": "front", "path": "cart/"},
    "checkout":        {"type": "front", "path": "checkout/"},
}


def _nav_url(nav: dict, store: str = "") -> str:
    """Resolve a nav-map entry to an absolute URL."""
    if nav["type"] == "carrier_registration":
        return woo_admin.carrier_registration_url(nav["carrier"], store or None)
    if nav["type"] == "front":
        return f"{woo_admin.site_url(store or None)}/{nav['path'].lstrip('/')}"
    return woo_admin.admin_url(nav.get("path", ""), store or None, **(nav.get("params") or {}))


def _navigate_in_app(page: "Page", destination: str, store: str = "") -> bool:
    """URL-based navigation for WooCommerce.

    WooCommerce admin has a real URL per screen, so there is no hamburger
    search or tab-index guessing the way the Shopify MCSL app needs. Unknown
    destinations fail loudly rather than landing somewhere arbitrary.
    """
    key = _normalise_nav_destination(destination)
    nav = _Woo_NAV_MAP.get(key)
    if nav is None:
        logger.warning("navigate: unknown destination %r", destination)
        return False

    try:
        page.goto(_nav_url(nav, store), wait_until="domcontentloaded")
...
_Woo_WORKFLOW_GUIDE = dedent("""\
## PluginHive WooCommerce Shipping Plugins — Key Workflows

The plugins run inside WordPress admin. There is **no app iframe and no single
endpoint**: every screen has its own URL, so navigate by URL, not by clicking
through menus.
...
### How to Navigate
  ...
  navigate: "orders"            → admin.php?page=wc-orders
  navigate: "products"          → edit.php?post_type=product
  navigate: "shipping"          → admin.php?page=wc-settings&tab=shipping
  navigate: "pluginsettings"    → admin.php?page=ph_multi_carrier_admin_menu
  navigate: "upsregistration"   → admin.php?page=ph_multi_carrier_ups_registration
  navigate: "status"            → admin.php?page=wc-status
  navigate: "plugins"           → plugins.php
  navigate: "shop" / "cart" / "checkout"  → storefront pages (buyer-side rates)

A single order opens at:
  admin.php?page=wc-orders&action=edit&id=<order_id>

### Login
WordPress session auth, not OAuth. `<site>/wp-login.php` with WP_ADMIN_USER /
WP_ADMIN_PASSWORD, or reuse the saved storage state. If a page redirects to
wp-login.php mid-run, the session expired — re-authenticate, do not retry blind.

### Automation Repo Grounding
The Playwright suite (`ups-woo-automation`) already has stable locators. Prefer
these before inventing selectors. Note the `ups_` / `wf_ups_` prefixes belong to
the standalone UPS plugin; the Multi-Carrier plugin uses its own prefix, so
probe the AX tree before assuming.

Orders list (`OrdersPage`):
- Order rows        : `#the-list`
- Order link        : `a.order-view`
- Bulk action select: `#bulk-action-selector-top`, apply with `#doaction`
- Success notice    : `.notice.notice-success`

Order edit screen — the plugin's shipment metabox:
- Generate Packages : `.button.ups_generate_packages`
- Calculate Rates   : `.button.wf_ups_generate_packages_rates`
- Packages built    : text "Step 2: Initiate your shipment."
- Package rows      : `#wf_ups_package_list tbody tr`  (count = packages; last row is the total)
- Service dropdown  : `#wf_ups_service_select`
- Confirm Shipment  : `.button.ups_create_shipment`
- Print Label       : link "Print Label"
- Print Return Label: link "Print Return Label"
- Void Shipment     : link "Void Shipment"
- Client Side Reset : link "Client Side Reset"
- Return service    : `#return_label_service` then link "Generate Return Label"

Plugin settings (`SettingsPage`) — select2 widgets, not plain selects:
- Packing method    : `#select2-woocommerce_wf_shipping_ups_packing_method-container`
- Label type        : `#select2-woocommerce_wf_shipping_ups_print_label_type-container`
- Open options list : `.select2-container--open .select2-results__option`
- Save              : button named "save"

### Label Generation Flow (single order, from the order screen)
1. navigate: "orders" → WooCommerce order list loads
2. Open the order (click `a.order-view`, or go straight to
   `admin.php?page=wc-orders&action=edit&id=<id>`)
3. Scroll to the plugin's shipment metabox
4. Click "Generate Packages" → package table renders
5. Click "Calculate Rates" → rates load; "Step 2: Initiate your shipment." appears
6. Pick the service in `#wf_ups_service_select`
7. Click "Confirm Shipment" → shipment is created with the carrier
8. Verify: "Print Label" link appears, and the order note records the tracking number

An order with N distinct products yields N package rows when packing is set to
per-item; with box packing the count depends on the box definitions. Read the
row count rather than assuming.

### Bulk Label Flow
1. navigate: "orders"
2. Tick the checkbox on each target order row
3. Choose the plugin's bulk action in `#bulk-action-selector-top`
4. Click `#doaction`
5. Verify `.notice.notice-success` contains, per order:
   "Order #<id>: Shipment accepted successfully. Labels are ready for printing."
6. A bulk PDF download is named `UPS-Shipping-Labels-YYYY-MM-DD.pdf`

### Return Label Flow
⚠️ Requires an order that already has a shipment. Use order_action:
existing_fulfilled in the plan.
1. Open the order edit screen
2. Choose the return service in `#return_label_service`
3. Click "Generate Return Label"
4. Verify the "Print Return Label" link appears

### Void Shipment Flow
1. Open an order that has a shipment
2. Click "Void Shipment"
3. In Test mode the plugin shows: "Please note that void is not possible in
   'Test' mode, as there is no real shipment is created with UPS." — that
   message is expected, not a bug
4. "Client Side Reset" clears labels locally so the shipment can be re-initiated;
   success note: "Client side reset of labels and shipment completed."

### Carrier Account Configuration
navigate: "pluginsettings" (Multi Carrier menu) or the per-carrier registration
screen (`upsregistration`, `fedexregistration`, `uspsregistration`).
...
- USPS  : Account Number / EasyPost key
- Canada Post: Customer Number, Contract ID, API key

Carrier registration cannot be scripted through the REST API — it is a manual
wp-admin step. If a scenario needs a carrier that is not registered, stop and
say so rather than trying to work around it.

### Storefront Rate Verification (buyer flow)
1. navigate: "shop" → add a product to the cart
2. navigate: "cart" → enter a destination in the shipping calculator
3. Read the rate rows the plugin returns (carrier name, service, amount)
4. navigate: "checkout" to confirm the same rates carry into checkout

This is the WooCommerce equivalent of Shopify carrier-service rate shopping and
is the only way to see what the buyer actually sees.

### Product-Level Shipping Fields
navigate: "products" → open a product → **Shipping** tab:
  weight, dimensions, shipping class.
PluginHive per-carrier product options (dry ice, alcohol, battery, signature,
insurance) appear as extra fields on the same tab once the plugin is active.
Unlike MCSL there is no separate in-app product screen — it is all one page.

### Debug / Request Logs
- WooCommerce → Status → Logs holds the plugin's request/response logs
- On the shipment-confirm screen the raw request and response render in the
  first two `<pre>` blocks on the page — request is `pre` nth(0), response nth(1)
- Enable "Debug Mode" in the plugin settings before a run that needs log evidence

---

## Document Verification Strategies

### DOC-01: Label Existence
After Confirm Shipment, verify the "Print Label" link is present on the order
screen and the order note records the tracking number.

### DOC-02: Label File Download
Click "Print Label" and capture the download.
- GIF labels start with the ASCII bytes `GIF`
- PNG labels start with `\x89PNG`
- ZPL content starts with `^XA`
- PDF content starts with `%PDF`
UPS tracking numbers in a label file match `1Z[A-Z0-9]{16}`.
Bulk downloads arrive as one PDF named `UPS-Shipping-Labels-YYYY-MM-DD.pdf`.

### DOC-03: Shipment Request / Response
On the shipment confirm log screen read the two `<pre>` blocks — request first,
response second — and check the expected fields are present. This is plain page
text, not a download.

### DOC-04: Packing Verification
Count `#wf_ups_package_list tbody tr` minus the total row, and compare against
the expected package count for the packing method under test.

### DOC-05: Rate Evidence Before Label Generation
Capture the rate table after "Calculate Rates" and before "Confirm Shipment" —
once the shipment is confirmed the rate list is replaced by the shipment result.
""")
...
class VerificationReport:
    ...
    def to_automation_context(self) -> str:
        ...
        lines = [f"=== Woo Smart AC Verification: {self.card_name} ===",
                 f"App: {self.app_url}", ""]
...
_DOMAIN_EXPERT_PROMPT = dedent("""\
    You are the domain expert for the Woo Multi-Carrier Shipping WooCommerce app.
    ...
    DOMAIN KNOWLEDGE (Woo docs / carrier API):
    {domain_context}
...
_PLAN_PROMPT = dedent("""\
    You are a QA engineer verifying a feature in the Woo Multi-Carrier Shipping WooCommerce App.
...
{woo_workflow_guide}
    ...
    Navigation rules (Woo has ONE endpoint — all navigation is click-based):
    - For label generation / order verification → nav_clicks: ["orders"]
    - For verifying an EXISTING label / documents → nav_clicks: ["orders"]
    - For carrier account configuration → nav_clicks: ["pluginsettings"] or the
      per-carrier registration destination ("upsregistration", "fedexregistration")
    - For product-level settings (dry ice, dimensions, services) → nav_clicks: ["appproducts"]
    - For creating/editing WooCommerce products → nav_clicks: ["wooproducts"]
    - For post-fulfillment WooCommerce order checks → nav_clicks: ["wooorders"]
    ...
    {{
      ...
      "nav_clicks": ["e.g. orders | carriers | appproducts | wooproducts | tracking"],
...
def _code_context(scenario: str, card_name: str) -> str:
    ...
    try:
        ...
        be_docs = search_code(query, k=2, source_type="woo_plugin") or []
        fe_docs: list = []
        try:
            fe_docs = search_code(query, k=2, source_type="woo_shipping_pro") or []
        except Exception:
            pass
...
def _ask_domain_expert(scenario: str, card_name: str, claude: "ChatAnthropic") -> str:
    ...
    Queries woo_knowledge (KB, wiki, sheets) and woo_code_knowledge (source code).
    ...
    _DOMAIN_SOURCES = [
        ("kb_articles",  query, "Woo Knowledge Base",              2),
    ...
    try:
        ...
        be_docs = search_code(query, k=2, source_type="woo_plugin")
        if be_docs:
            code_parts.append("\n---\n".join(
                f"[{d.metadata.get('file_path', '').split('/')[-1]}]\n{d.page_content[:240]}"
                for d in be_docs[:2]
...
def _ax_tree(page: Any, max_lines: int = 250) -> str:
    ...
    iframe the way the Shopify MCSL app has one, so the main frame carries
    almost everything. Child frames are still walked (label previews, carrier
    registration modals, and some payment screens use them), minus the obvious
    analytics/advert frames.
    """
    lines: list[str] = []

    # 1. Main page snapshot — wp-admin chrome plus the plugin settings screen
    try:
    ...
    # 2. Child frames — skip trackers and blank frames
    try:
        for frame in page.frames:
            ...
            if not frame_url or frame_url.startswith("about:"):
                continue
            if any(noise in frame_url for noise in _FRAME_NOISE):
                continue
            try:
...
_FRAME_NOISE = (
    ...
    "hotjar", "intercom", "recaptcha", "youtube.com", "gravatar",
)
...
def _network(page: Any, patterns: list[str] | None = None) -> str:
    ...
    Checks the main page plus any non-tracking child frame, so admin-ajax and
    REST calls are captured wherever they were issued.
    ...
    # Child frames (same filter as _ax_tree)
    try:
        for frame in page.frames:
            if frame is page.main_frame:
                continue
    ...
    if patterns:
        ...
        hits = [
            e for e in hits
            if "admin-ajax.php" in e or "/wp-json/" in e or "admin.php" in e
            or "pluginhive" in e.lower()
        ]
...
def _get_app_frame(page: Any) -> Any:
    """Return the locator root for plugin UI.

    WooCommerce plugin settings, the order metabox, and the shipment log all
    render inline in wp-admin, so the root is the page itself — unlike the
    Shopify MCSL app, which puts everything inside iframe[name="app-iframe"].

    Kept as a function (rather than inlining `page`) because a few plugin
    screens — carrier registration modals, label previews — do use a child
    frame. When exactly one non-tracking child frame is present, that is
    returned instead.
    """
    try:
        candidates = [
            ...
            and not any(noise in (f.url or "") for noise in _FRAME_NOISE)
        ]
        if len(candidates) == 1:
            return candidates[0]
    except Exception:
...
def _preflight_open_order_summary(page: Any, result: ScenarioResult, order_id: str) -> bool:
    """Open a WooCommerce order's edit screen.

    WooCommerce orders are URL-addressable, so the reliable path is a direct
    goto rather than filtering a grid and clicking a link. The click strategies
    are kept as a fallback for the case where the order id is unknown but its
    row is on screen.
    """
    numeric_id = str(order_id).lstrip("#").strip()
    page.wait_for_timeout(500)

    if numeric_id.isdigit():
        try:
            page.goto(woo_admin.order_url(numeric_id), wait_until="domcontentloaded")
            page.wait_for_timeout(1000)
            logger.info("Preflight opened order %s by URL", numeric_id)
            _record_macro_step(
                result, action="navigate",
                description=f"Preflight opened order {numeric_id} by URL",
                target=numeric_id, selector=woo_admin.order_url(numeric_id),
                locator_source="direct_url", success=True, page=page,
            )
            return True
        except Exception as e:
            logger.debug("Preflight direct order URL failed for %s: %s", numeric_id, e)

    # Fallback: click the row link on the orders list (a.order-view).
    frame = _get_app_frame(page)
    link_val = f"#{numeric_id}" if numeric_id else str(order_id)
    strategies: list[tuple[str, Any]] = [
        ("order_view_link", lambda: frame.locator(f'a.order-view:has-text("{numeric_id}")').first.click(timeout=8000)),
        ("getByRole_link", lambda: frame.get_by_role("link", name=link_val).first.click(timeout=5000)),
        ("has_text", lambda: frame.locator(f'a:has-text("{link_val}")').first.click(timeout=5000)),
    ]
    for name, click_fn in strategies:
        try:
            click_fn()
            page.wait_for_timeout(1500)
            logger.info("Preflight clicked order %s via strategy: %s", link_val, name)
            _record_macro_step(
                result, action="click",
                description=f"Preflight clicked order {link_val} ({name})",
                target=link_val, selector=name,
                locator_source=name, success=True, page=page,
            )
            return True
        except Exception:
            continue
    ...
    _record_macro_step(
        result, action="click",
        description=f"Preflight could not open order {link_val}",
        target=link_val, selector="", locator_source="", success=False, page=page,
    )
    return False
...
def _preflight_prepare_and_generate_label(page: Any, result: ScenarioResult) -> bool:
    """Drive the plugin's shipment metabox: packages → rates → confirm.

    Locators mirror ups-woo-automation/src/pages/wooCommerceAdmin/ordersPage.ts.
    The `ups_` / `wf_ups_` prefixes belong to the standalone UPS plugin, so each
    step also tries a role/text match that works for the Multi-Carrier plugin.

    Rate evidence is captured between Calculate Rates and Confirm Shipment —
    confirming replaces the rate list with the shipment result.
    """
    frame = _get_app_frame(page)

    def _step(name: str, selectors: list[tuple[str, str, Any]]) -> bool:
        ok, selector, source = _macro_click(page, name=name, selectors=selectors)
        _record_macro_step(
            result, action="click",
            description=f"Preflight clicked {name}",
            target=name, selector=selector,
            locator_source=source or "macro_click",
            success=ok, page=page,
        )
        return ok

    ok_packages = _step("Generate Packages", [
        ("css_plugin", '.button.ups_generate_packages',
         lambda: frame.locator(".button.ups_generate_packages").first.click()),
        ("role_button", 'getByRole("button", { name: /generate packages/i })',
         lambda: frame.get_by_role("button", name=re.compile("generate packages", re.I)).first.click()),
    ])

    ok_rates = _step("Calculate Rates", [
        ("css_plugin", '.button.wf_ups_generate_packages_rates',
         lambda: frame.locator(".button.wf_ups_generate_packages_rates").first.click()),
        ("role_button", 'getByRole("button", { name: /calculate rates/i })',
         lambda: frame.get_by_role("button", name=re.compile("calculate rates", re.I)).first.click()),
    ])

    if ok_rates:
        # Rates only exist in this window — grab them before confirming.
        try:
            page.wait_for_timeout(1500)
            rate_text = frame.locator("#wf_ups_service_select").first.inner_text(timeout=5000)
            if rate_text and result.steps:
                result.steps[-1].captured_artifact = rate_text
        except Exception:
            logger.debug("Could not capture the rate list before confirming")

    ok_confirm = _step("Confirm Shipment", [
        ("css_plugin", '.button.ups_create_shipment',
         lambda: frame.locator(".button.ups_create_shipment").first.click()),
        ("role_button", 'getByRole("button", { name: /confirm shipment/i })',
         lambda: frame.get_by_role("button", name=re.compile("confirm shipment", re.I)).first.click()),
    ])

    return ok_confirm or (ok_packages and ok_rates)


def _preflight_download_documents(page: Any, result: ScenarioResult) -> bool:
    """Download the generated label.

    WooCommerce has no "Download Documents" ZIP the way the Shopify MCSL app
    does — the plugin exposes a per-shipment "Print Label" link, and the bulk
    action returns a single combined PDF.
    """
    frame = _get_app_frame(page)
    ok, selector, source = _macro_click(
        page,
        name="Print Label",
        selectors=[
            ("role_link_exact", 'getByRole("link", { name: "Print Label" })',
             lambda: frame.get_by_role("link", name="Print Label").first.click()),
            ("text_exact", 'getByText("Print Label", { exact: true })',
             lambda: frame.get_by_text("Print Label", exact=True).first.click()),
        ],
    )
    _record_macro_step(
        result,
        action="click",
        description="Preflight clicked Print Label",
        target="Print Label",
        selector=selector,
        locator_source=source or "macro_click",
        success=ok,
...
def _preflight_open_print_documents(page: Any, result: ScenarioResult) -> bool:
    """Open the shipment request/response log.

    The plugin renders the raw carrier request and response in the first two
    <pre> blocks on the shipment-confirm screen. Debug Mode must be on in the
    plugin settings or the blocks are empty.
    """
    frame = _get_app_frame(page)
    try:
        blocks = frame.locator("pre")
        captured = []
        for i in range(min(2, blocks.count())):
            captured.append(blocks.nth(i).inner_text(timeout=5000))
        ok = bool(captured)
        if ok and result.steps:
            result.steps[-1].captured_artifact = "\n\n---\n\n".join(captured)
    except Exception as e:
        logger.debug("Could not read the shipment log blocks: %s", e)
        ok = False

    _record_macro_step(
        result,
        action="observe",
        description="Preflight read the shipment request/response log",
        target="shipment log <pre> blocks",
        selector="pre",
        locator_source="css",
        success=ok,
        page=page,
...
def _preflight_open_product_settings(page: Any, result: ScenarioResult) -> bool:
    frame = _get_app_frame(page)
    selectors = _automation_hint_selectors("woo products product settings edit row", action="click", frame=frame)
    selectors.extend([
...
def _preflight_open_add_carrier(page: Any, result: ScenarioResult) -> bool:
    frame = _get_app_frame(page)
    selectors = _automation_hint_selectors("woo carriers add carrier button", action="click", frame=frame)
    selectors.extend([
...
def _preflight_open_automation_rule(page: Any, result: ScenarioResult) -> bool:
    frame = _get_app_frame(page)
    selectors = _automation_hint_selectors("woo automation add rule create rule", action="click", frame=frame)
    selectors.extend([
...
def _preflight_open_general_settings(page: Any, result: ScenarioResult) -> bool:
    frame = _get_app_frame(page)
    selectors = _automation_hint_selectors("woo general settings packaging settings row", action="click", frame=frame)
    selectors.extend([
        ("css_locator", 'table tbody tr:first-child a', lambda: frame.locator('table tbody tr:first-child a').first.click()),
        ("role_link_first", 'getByRole("link").first()', lambda: frame.get_by_role("link").first.click()),
    ])
...
def _preflight_fill_named_field(page: Any, result: ScenarioResult, label: str, value: str) -> bool:
    frame = _get_app_frame(page)
    selectors = _automation_hint_selectors(f"woo products {label}", action="fill", frame=frame, value=value)
    selectors.extend([
...
def _preflight_save_form(page: Any, result: ScenarioResult, *, description: str) -> bool:
    frame = _get_app_frame(page)
    selectors = _automation_hint_selectors("woo save button product settings automation carriers general settings", action="click", frame=frame)
    selectors.extend([
...
def _run_preflight_setup(
    *,
    ...
    nav_clicks = _dedupe_strings([
        ...
        if _normalise_nav_destination(str(v)) in _Woo_NAV_MAP
    ])
    ...
    if order_id and (_orders_in_plan or _orders_in_scenario):
        ...
        if opened_order:
            setup_notes.append(f"Opened the order screen for test order {order_id} during preflight.")
        else:
            setup_notes.append(f"Could not open the order screen for test order {order_id} during preflight.")
    ...
    if ("view log" in lower or "request log" in lower or "rate req" in lower or "label req" in lower) and "orders" not in nav_clicks:
        ...
        if ok and order_id:
            opened_order = _preflight_open_order_summary(page, result, order_id)
            if opened_order:
                setup_notes.append(f"Opened the order screen for test order {order_id} after request/log preflight navigation.")
    ...
    if ("commercial invoice" in lower or "cn22" in lower or "print documents" in lower or "download documents" in lower) and "orders" not in nav_clicks:
        ...
        if ok and order_id:
            opened_order = _preflight_open_order_summary(page, result, order_id)
            if opened_order:
                setup_notes.append(f"Opened the order screen for test order {order_id} after document preflight navigation.")
    ...
    if order_id and any(token in lower for token in ("generate label", "label created", "mark as fulfilled", "shipment", "shipping label")):
        ...
        if triggered:
            setup_notes.append("Ran Generate Packages / Calculate Rates / Confirm Shipment during preflight.")
    ...
    if "print documents" in lower:
        ...
        if printed:
            setup_notes.append("Read the shipment request/response log during preflight.")
        else:
            setup_notes.append("Could not read the shipment log during preflight.")
...
_DECISION_PROMPT = dedent("""\
    You are verifying this AC scenario in the Woo Multi-Carrier Shipping WooCommerce App.
...
{woo_workflow_guide}
    ...
    ── WooCommerce admin ──
    - "orders"           → WooCommerce > Orders list (admin.php?page=wc-orders)
    - "products"         → Products list (edit.php?post_type=product)
    - "newproduct"       → Add new product
    - "status"           → WooCommerce > Status (versions, logs)
    - "plugins"          → Plugins list (is the build active?)
    - "dashboard"        → wp-admin home
    ── WooCommerce settings ──
    - "settings"         → WooCommerce > Settings
    - "generalsettings"  → Settings > General (store address, currency, units)
    - "shipping"         → Settings > Shipping (zones and methods)
    - "shippingclasses"  → Settings > Shipping > Classes
    ── PluginHive plugin screens ──
    - "pluginsettings"   → Multi Carrier plugin menu
    - "carriers"         → Multi Carrier plugin menu (carrier list)
    - "upsregistration" / "fedexregistration" / "uspsregistration"
                         → per-carrier registration screen
    ── Storefront (buyer-side rate checks) ──
    - "shop" / "cart" / "checkout"
    ...
    - wp-admin is URL-addressable: navigate straight to a destination. There is
      NO app iframe, NO single app endpoint, and NO hamburger search here.
    - action=observe on first step to capture visible elements before interacting
    - Do NOT explore unrelated sections of the admin
    - If a page redirects to wp-login.php, the session expired — say so rather
      than retrying blind

    PRODUCT SETTINGS ARE ALL ON ONE PAGE:
    - navigate: "products" → open the product → "Shipping" tab in Product data.
      Weight, dimensions, and shipping class live there, and the plugin adds its
      per-carrier fields (dry ice, alcohol, battery, signature, declared value)
      to the same tab once active. Unlike the Shopify MCSL app there is no
      separate in-app product screen.

    Label generation (same flow for ALL carriers):
    1. navigate: "orders" → WooCommerce orders list
    2. Open the test order — click a.order-view, or go directly to
       admin.php?page=wc-orders&action=edit&id=<order_id>
    3. Scroll to the plugin's shipment metabox
    4. Click "Generate Packages" → the package table renders
    5. Click "Calculate Rates" → wait for "Step 2: Initiate your shipment."
    6. Select the service in #wf_ups_service_select
       ⚠️ Capture rate evidence NOW — confirming replaces the rate list
    7. Click "Confirm Shipment"
    8. Verify the "Print Label" link appears and the order note has the tracking number

    Bulk labels: select the order rows, choose the plugin's bulk action in
    #bulk-action-selector-top, click #doaction, and read .notice.notice-success.
""")

_AUTOMATION_WORKFLOW_QUERIES: dict[str, tuple[str, ...]] = {
    "orders": (
        "OrdersPage the-list order-view selectOrderInWSSOrdersPage goto wc-orders",
        "bulk-action-selector-top doaction notice-success Shipment accepted successfully",
    ),
    "order_shipment": (
        "ups_generate_packages wf_ups_generate_packages_rates wf_ups_service_select ups_create_shipment",
        "printLabelInWSSOrdersPage Print Label verifyShipmentConfirmLog shipmentConfirmRequestPre",
    ),
    "return_label": (
        "return_label_service Generate Return Label Print Return Label",
        "voidShipmentInWSSOrdersPage Client Side Reset warningTextVoidShipment",
    ),
    "packaging": (
        "wf_ups_package_list numberOfPackagesInOrdersPage packing method box packing",
        "select2-woocommerce_wf_shipping_ups_packing_method-container parcelPackingDropdown",
    ),
    "settings": (
        "SettingsPage selectTab selectLabelTypeOption saveChangesBtn",
        "select2-woocommerce_wf_shipping_ups_print_label_type-container wc-settings tab=shipping",
    ),
    "products": (
        "ShopPage storeProducts loadStoreProducts productSetupController ensureStoreProducts",
        "createWooOrder simple variable digital product weight dimensions shipping class",
    ),
    "carriers": (
        "ph_multi_carrier_registration carrier account credentials client id secret",
    ),
    "status": (
        "StatusPage wc-status active_plugins system status logs debug",
    ),
    "storefront": (
        "shopPage cart checkout shipping calculator rate rows",
    ),
}
...
def _automation_training_queries(scenario: str, card_name: str) -> list[str]:
    ...
    for tag, phrases in {
        "orders": ("order", "order list", "bulk", "order grid"),
        "order_shipment": ("label", "shipment", "confirm shipment", "generate label",
                           "tracking", "print label", "rate"),
        "return_label": ("return label", "void", "reset", "cancel shipment"),
        "packaging": ("packaging", "package", "box packing", "pack item", "parcel"),
        "settings": ("setting", "configuration", "label type", "debug mode", "zone", "method"),
        "products": ("product", "variation", "sku", "dimension", "weight",
                     "dry ice", "alcohol", "battery", "signature"),
        "carriers": ("carrier", "account", "credential", "registration"),
        "status": ("version", "plugin active", "system status", "log"),
        "storefront": ("cart", "checkout", "shop", "buyer", "shipping calculator"),
    }.items():
        if any(phrase in combined for phrase in phrases):
            tags.append(tag)

    queries = [
        "BasePage LoginPage HomePage wp-admin goto wooCommerceAdmin",
...
def verify_ac(
    ...
    """Verify AC scenarios for a card against the live WooCommerce plugin.
    ...
    Args:
        ...
        app_url:        Full Woo app URL in WordPress admin (auto-built from config.STORE if empty)
        ...
        qa_platform_confirmed: Allow WooCommerce execution for a card that names
    ...
    if not app_url:
        _slug = _store_slug()
        if _slug:
            app_url = woo_admin.plugin_settings_url()
...
def build_smart_context(card_name: str, tc_markdown: str = "") -> str:
    ...
    knowledge snapshot:
      ...
      - Backend server code (woo_pluginsaas) relevant to the feature
    ...
    _CODE_SOURCES = [
        ...
        ("woo_plugin", f"{card_name} shipping label carrier",          3, "Backend: server-side logic"),
    ]
...
def _run_scenario_verification(
    *,
    ...
    try:
        # Navigate to the app URL so the plugin settings screen loads before
