...
def _app_behaviour_research(query: str, *, max_docs: int) -> str:
    ...
    if not sections:
        return ""
    return "Woo app behaviour findings from local RAG:\n" + "\n\n".join(section for section in sections if section.strip())
...
def _code_research(query: str) -> str:
    try:
        ...
        for source_type, label, limit in (
            ...
            ("woo_plugin", "Woo server code findings", 2),
            ("woo_shipping_pro", "Woo client code findings", 2),
        ):
...
def _build_requirement_research_context_cached(
    ...
    if not parts:
        return (
            ...
            "No additional Woo research findings were available. Use existing product and code context, "
    ...
    return (
        ...
        "2. Woo app behaviour context explains how the product currently exposes those rules.\n"
