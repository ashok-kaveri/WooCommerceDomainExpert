...
PLATFORM_KEYWORDS: dict[str, tuple[str, ...]] = {
    "WooCommerce": ("woocommerce", "woo commerce", "wordpress", "wp-admin",
                    "ph multi carrier shipping", "woo plugin"),
    "Shopify": ("shopify", "shopify admin", "mcsl", "multi carrier shipping label app"),
    "BigCommerce": ("bigcommerce", "big commerce"),
    "Magento": ("magento", "magento 2", "magento marketplace"),
    "PrestaShop": ("prestashop", "presta shop"),
}
# Platforms this repo's AI QA browser and Playwright stack cannot execute.
# WooCommerce is the native platform here, so it is deliberately absent.
NON_WOO_PLATFORMS = {"Shopify", "BigCommerce", "Magento", "PrestaShop"}
...
AC_WRITER_PROMPT = """\
You are a senior QA lead for PluginHive Woo (Multi-Carrier Shipping Labels) on WordPress/WooCommerce.
The app supports FedEx, UPS, DHL, USPS, and other carriers.
Navigation must match the detected customer/test platform from the card or ticket. Default to WooCommerce when no platform is explicit; do not use WordPress admin steps for WooCommerce, BigCommerce, Magento, or PrestaShop cards.
...
- If the card/ticket identifies WooCommerce, BigCommerce, Magento, or PrestaShop, write AC around that platform's store/admin/checkout flow and mention shared Woo implementation only as a scope note.
...
Each AC must be testable via the Woo app UI.
"""

TEST_CASE_PROMPT = """\
You are a senior QA engineer for PluginHive Woo (Multi-Carrier Shipping Labels) on WordPress/WooCommerce.
The app supports FedEx, UPS, DHL, USPS, and other carriers.
Navigation must match the detected customer/test platform from the card or ticket. Default to WooCommerce when no platform is explicit; do not use WordPress admin steps for WooCommerce, BigCommerce, Magento, or PrestaShop cards.

Work from real Woo behaviour, not generic shipping assumptions.
...
- Keep steps executable in the detected platform admin/storefront, the Woo app, request/response logs, or API flow where relevant.
...
You are reviewing generated QA test cases for PluginHive's WooCommerce shipping plugins on WordPress/WooCommerce.
...
- platform drift, especially WordPress admin steps on WooCommerce, BigCommerce, Magento, or PrestaShop cards
...
AC_REVIEW_PROMPT = """\
You are reviewing generated Acceptance Criteria for PluginHive's WooCommerce shipping plugins on WordPress/WooCommerce.
...
- platform drift, especially WordPress admin assumptions when the card/ticket is WooCommerce, BigCommerce, Magento, or PrestaShop
...
def detect_platform_scope(*texts: str) -> list[str]:
    """Detect the customer/test platform, defaulting to WooCommerce when not explicit."""
...
def platform_scope_brief(*texts: str) -> str:
    ...
    if platforms:
        ...
        return (
            ...
            "preconditions. If Shopify, BigCommerce, Magento, or PrestaShop is explicitly detected, do not use "
            "WordPress admin steps. If no non-WooCommerce platform is detected, WooCommerce is the default "
            "QA/support platform. The feature can still be treated as shared PluginHive behavior unless the "
            "card limits the scope."
        )


def woo_automation_scope(*texts: str, qa_confirmed: bool = False) -> tuple[bool, list[str], str]:
    ...
    AI QA browser and Playwright automation stack in this repo run only against
    the WooCommerce plugins and the ups-woo-automation repo. If a card names
    another platform, QA must confirm whether a WooCommerce-equivalent execution
    is acceptable, because most PluginHive shipping behaviour is shared across
    platforms.
    """
    platforms = detect_platform_scope(*texts)
    non_woo = [platform for platform in platforms if platform in NON_WOO_PLATFORMS]
    if non_woo:
        joined = ", ".join(non_woo)
        if qa_confirmed:
            return (
                True,
                platforms,
                f"QA confirmed execution for {joined}. Run the available WooCommerce AI QA or automation flow, "
                f"but keep the reported platform ({joined}) visible in evidence and comments.",
            )
        return (
            False,
            platforms,
            f"This card mentions {joined}. Ask QA whether to execute it through the available WooCommerce flow. "
            f"If QA confirms, run AI QA/automation with {joined} noted as the reported customer platform.",
        )
    return True, platforms, "WooCommerce AI QA/automation execution is supported."
...
def _build_generation_brief(
    ...
    lines = [
        ...
        "2. Requirement research provides Woo carrier/platform, wiki, backlog, and app-behavior facts.",
        "3. Use only supported Woo flows and avoid inventing carrier-only rules without evidence.",
        "",
...
def _build_code_context_section(card_name: str, card_desc: str) -> str:
    """Query indexed Woo code for automation and plugin source app context."""
    ...
    try:
        ...
        _source_specs = [
            ...
            for st, lbl, lim, sl in (
                ...
                ("woo_plugin","the Multi-Carrier plugin repo implementation",     3, 600),
                ("woo_shipping_pro","the Shipping Pro repo implementation",     2, 500),
            )
            if stats.get(st, 0) > 0
        ]
