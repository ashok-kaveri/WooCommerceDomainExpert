...
def test_woo_generation_brief_prioritizes_linked_handoff_over_comment_refs():
...
def test_woo_prerequisites_ignore_label_generation_when_only_comments_mention_it():
...
def test_mc01_audit_comment():
    ...
    with patch("requests.post", return_value=mock_response) as mock_post:
        from pipeline.trello_client import TrelloClient
        client = TrelloClient(api_key="k", token="t", board_id="b")
        client.add_comment("card123", "Moved by Woo QA Pipeline")
...
def test_mc01_get_card_returns_rich_trello_card():
    ...
    card_json = {
        ...
        "idList": "list1",
        "labels": [{"name": "SL: WooCommerce Woo"}, {"name": "SL: International & Customs"}],
        "idMembers": ["m1"],
    }
    ...
    assert card.id == "card123"
    assert card.labels == ["SL: WooCommerce Woo", "SL: International & Customs"]
...
def test_rqa05_card_summary_combined_text_includes_comments_labels_and_checklists():
    ...
    card = CardSummary(
        ...
        card_labels=["SL: International & Customs", "SL: WooCommerce Woo"],
...
def test_rqa04_append_to_sheet_ai_tab_layout_mapping():
    """append_to_sheet() writes Woo AI tab rows using the live 12-column layout."""
    header_row = [[
        "Sl no",
        "Epics ( Title)",
        "Scenario",
...
Then tracking_number is clean""",
    ...
    assert appended[11] == "Priority: High | Release: Woo 378"
...
def test_rqa01_validate_card_accepts_content_blocks():
    ...
    fake_response.content = [
        {
            "type": "text",
            "text": '{"overall_status":"PASS","summary":"Looks good","requirement_gaps":[],"ac_gaps":[],"accuracy_issues":[],"suggestions":[],"kb_insights":"Grounded in Woo context."}',
        }
    ]
...
def test_slack_toggle_detection_handles_store_enablement_and_unquoted_woo_keys():
    ...
    toggles = detect_toggles(
        ...
        "\n".join([
            ...
            "woo.feature.minimum.customs.value.floor.enabled",
        ]),
    )
...
_AUTO01_FAKE_RESPONSE = (
    ...
    "  test.skip(!['woo-automation'].includes(process.env.WOO_SITE_URL ?? ''), 'skip');\n"
...
def test_auto01_non_woo_platform_skips_automation_before_llm():
    """Automation asks for QA confirmation before running BigCommerce cards through WooCommerce flow."""
    from unittest.mock import patch
    from pipeline.automation_writer import write_automation
...
def test_smart_ai_qa_non_woo_returns_manual_block():
    """AI QA asks QA before executing a non-WooCommerce reported card through WooCommerce flow."""
...
def test_auto04_find_pom_matches_existing_woo_files(tmp_path):
    repo = tmp_path / "woo-auto"
    ...
    with patch.object(aw_mod.config, "WOO_AUTOMATION_REPO_PATH", str(repo)):
...
def test_auto05_write_automation_updates_existing_woo_files(tmp_path):
    repo = tmp_path / "woo-auto"
    (repo / "support/pages/orders").mkdir(parents=True)
    (repo / "tests/orderSummary").mkdir(parents=True)
    (repo / "support/pages/orders/orderSummaryPage.ts").write_text("class OrderSummary {}", encoding="utf-8")
...
def test_auto07_post_results_formats_and_posts_to_slack():
    ...
    with patch("pipeline.slack_client._make_client", return_value=mock_client):
        result = post_results(run_result, "WooApp 1.2.3")

    assert result["ok"] is True
    assert result["ts"] == "123.456"
    payload = mock_client.post_to_channel.call_args[0][0]
    assert "Woo Automation — WooApp 1.2.3" in payload
...
def test_sheets_writer_lists_live_tabs_from_configured_sheet():
    ...
    fake_ws1 = types.SimpleNamespace(title="Woo Master")
    ...
    assert tabs == ["Woo Master", "Shipping Labels"]
