...
def test_ui05_branding():
    ...
    assert calls.get("page_title") == "Woo QA Pipeline"
...
def test_ui04_knowledge_base():
    ...
    import inspect
    src = inspect.getsource(pd)
    # Single "Woo App Code" section replaces separate backend/frontend
    assert "woo_plugin" in src
    assert "WOO_AUTOMATION_REPO_PATH" in src
    # Wiki section added
...
def test_dashboard_wiki_has_pull_sync_and_full_reindex_controls():
    ...
    assert 'with st.expander("📖 Woo Wiki")' in src
