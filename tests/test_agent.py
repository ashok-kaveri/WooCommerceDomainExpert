...
def test_ax_tree_capture():
    ...
    # -- _ax_tree: main wp-admin frame plus one plugin child frame --
    mock_frame = MagicMock()
    mock_frame.url = "https://test-site.example.com/wp-admin/admin.php?page=ph_multi_carrier_admin_menu"
    ...
    assert isinstance(result, str)
    assert len(result) > 0
    # Main-frame content always lands — WooCommerce settings render inline.
    assert "Main Nav Button" in result
    # Non-tracking child frames are still walked and labelled.
    assert "--- [FRAME:" in result, f"Expected FRAME header in ax_tree output, got: {result!r}"
    assert "Generate Label" in result

    # Analytics frames are skipped rather than walked.
    noisy = MagicMock()
    noisy.url = "https://www.googletagmanager.com/ns.html"
    noisy.accessibility.snapshot.return_value = {"role": "button", "name": "Tracker", "children": []}
    mock_page.frames = [mock_page.main_frame, noisy]
    assert "Tracker" not in _ax_tree(mock_page)
...
def test_action_handlers():
    ...
    app_base = "https://test-site.example.com/wp-admin/admin.php?page=ph_multi_carrier_admin_menu"
    ...
    # navigate with a named destination → _navigate_in_app goes straight to the
    # wp-admin URL; WooCommerce admin is URL-addressable, so every destination
    # is a goto rather than a click chain.
    result_nav = _do_action(mock_page, {"action": "navigate", "url": "wooorders"}, app_base)
    assert isinstance(result_nav, bool)
    assert mock_page.goto.called
    call_url = mock_page.goto.call_args[0][0]
    assert "/wp-admin/admin.php" in call_url
    assert "page=wc-orders" in call_url

    # plugin settings resolve to the Multi-Carrier admin menu
    mock_page.goto.reset_mock()
    _do_action(mock_page, {"action": "navigate", "url": "pluginsettings"}, app_base)
    assert "page=ph_multi_carrier_admin_menu" in mock_page.goto.call_args[0][0]

    # an unknown destination fails instead of landing somewhere arbitrary
    mock_page.goto.reset_mock()
    assert _do_action(mock_page, {"action": "navigate", "url": "nowhere"}, app_base) is False
    assert not mock_page.goto.called
...
def test_plan_scenario_normalizes_navigation_aliases():
    """AI QA planning should map QA/model wording to supported Woo nav keys."""
...
def test_order_creator(tmp_path):
    """AGENT-04 (order): create_order posts a WooCommerce REST order and returns its number."""
    from unittest.mock import MagicMock, patch
    ...
    env_file.write_text(
        ...
        'SIMPLE_PRODUCTS_JSON=[{"product_id": 123, "variation_id": 456}]\n',
        encoding="utf-8",
    )

    fake_client = MagicMock()
    fake_client.create_order.return_value = {"id": 9999, "number": "9999"}
    ...
    with patch("pipeline.order_creator._client_for_env", return_value=fake_client):
        ...
        assert env_data.get("WOO_CONSUMER_SECRET") == "cs_fake"
        assert env_data.get("WOO_SITE_URL") == "https://qa.example.com"
        assert "SIMPLE_PRODUCTS_JSON" in env_data

        order_id = create_order(env_file)
        assert order_id == "9999"
        assert fake_client.create_order.called

        payload = fake_client.create_order.call_args[0][0]
        assert payload["set_paid"] is True
        assert payload["line_items"] == [{"product_id": 123, "variation_id": 456, "quantity": 1}]
        assert payload["billing"]["postcode"] == "60601"
...
def test_verify_scenario_order_wiring():
    ...
    with patch("pipeline.smart_ac_verifier._decide_next", return_value=verify_action), \
         patch("pipeline.order_creator._client_for_env") as mock_client_factory:

        mock_client = MagicMock()
        mock_client.create_order.return_value = {"id": 123, "number": "ORD-123"}
