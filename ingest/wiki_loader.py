...
Wiki Loader
===========
Reads all Markdown files from the Woo wiki directory and returns chunked
LangChain Documents ready for indexing into ChromaDB (woo_knowledge).

Woo wiki folder structure maps to categories:
...
# Woo folder-name → human-readable category label
...
def load_wiki_docs(wiki_path: str | None = None) -> list[Document]:
    """
    Walk the Woo wiki directory, read every Markdown/text file, chunk the content,
    ...
    if not wiki_root.exists():
        logger.warning(
            ...
            "Set WIKI_PATH env var to the woo-wiki directory.",
            wiki_root,
        )
        return []
